...
export function ProfileEditor({ initialProfile }: Props) {
  ...
  const [data, setData] = useState<PartialProfile>(initialProfile)
    ...
    <div className="max-w-2xl mx-auto py-10 px-4">
      <div className="mb-6">
        ...
        <p className="text-muted-foreground text-sm mt-1">
          Update your information and agent settings
        </p>
      </div>
      ...
      <div className="space-y-6">
        ...
        {step === 0 && (
          <StepPersonal
            ...
            onNext={(v) => { handleStepData({ personal: v }); setStep(1) }}
          />
        )}
        {step === 1 && (
          <StepObjective
            ...
            onBack={() => setStep(0)}
            onNext={(v) => { handleStepData({ objective: v }); setStep(2) }}
          />
        )}
        {step === 2 && (
          <StepExperiences
            ...
            onBack={() => setStep(1)}
            onNext={(v) => { handleStepData({ experiences: v }); setStep(3) }}
          />
        )}
        {step === 3 && (
          <StepEducation
            ...
            onBack={() => setStep(2)}
            onNext={(v) => { handleStepData({ education: v }); setStep(4) }}
          />
        )}
        {step === 4 && (
          <StepSkills
            ...
            onBack={() => setStep(3)}
            onNext={(v) => { handleStepData({ skills: v }); setStep(5) }}
          />
        )}
        {step === 5 && (
          <StepAgentConfig
