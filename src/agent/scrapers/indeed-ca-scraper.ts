...
export class IndeedCAScraper extends BaseScraper {
  async scrape(): Promise<RawJobInput[]> {
    ...
    try {
      ...
      for (let pageNum = 0; pageNum < maxPages && jobs.length < this.config.maxJobsPerRun; pageNum++) {
        ...
        const url = `https://ca.indeed.com/jobs?q=software+developer&l=Canada&sort=date&fromage=7${start > 0 ? `&start=${start}` : ''}`

        this.logger.info('indeed_ca_fetch_page', { page: pageNum + 1, url })
        ...
        try {
          ...
          if (pageJobs.length === 0) {
            this.logger.info('indeed_ca_no_jobs', { page: pageNum + 1 })
            break
          }
          ...
          for (const card of pageJobs) {
            ...
            jobs.push({
              ...
              location: card.formattedLocation ?? 'Canada',
              url: `https://ca.indeed.com/viewjob?jk=${card.jobkey}`,
              description,
          ...
          this.logger.info('indeed_ca_page_done', { page: pageNum + 1, collected: jobs.length })

          if (pageNum < maxPages - 1 && jobs.length < this.config.maxJobsPerRun) {
            await page.waitForTimeout(10000 + Math.random() * 10000)
          }
        } catch (err) {
          this.logger.error('indeed_ca_page_error', { page: pageNum + 1, error: String(err) })
          break
        }
      }
      ...
      this.logger.error('indeed_ca_scraper_failed', { error: String(err) })
    } finally {
      await browser.close()
    }

    this.logger.info('indeed_ca_scrape_complete', { total: jobs.length })
  ...
  override async scrapeJobDetail(url: string): Promise<string | null> {
    ...
    try {
      ...
      this.logger.warn('indeed_ca_detail_failed', { url, error: String(err) })
      return null
    } finally {
      await browser.close()
    }
